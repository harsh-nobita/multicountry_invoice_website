...
function recalculate() {
  ...
  // Discount (applied to subtotal BEFORE tax is calculated)
  let discountAmt = 0;
  ...
  // Taxable amount = subtotal minus discount. All taxes are calculated on this amount.
  const afterDiscount = subtotal - discountAmt;

  // Show the "Taxable Amount" row only when a discount AND at least one tax line exist —
  // this makes it visually clear that tax is computed on the post-discount amount.
  const taxableRow = document.getElementById('taxableRow');
  if (taxableRow) {
    const showTaxable = state.showDiscount && discountAmt > 0 && state.taxLines.length > 0;
    taxableRow.style.display = showTaxable ? 'flex' : 'none';
    if (showTaxable) document.getElementById('taxableVal').textContent = fmt(afterDiscount);
  }

  // Tax lines — always calculated on afterDiscount (taxable amount)
  let totalTax = 0;
...
function collectInvoiceData() {
  // Grab first client-name editable (2nd contenteditable overall = first in Bill To section)
  const clientNameEl = document.querySelector('.inv-party .editable[contenteditable]');
  const grandTotalText = document.getElementById('grandTotalVal')?.textContent || '';
...
// ── PRINT ──────────────────────────────────────
// Print only the invoice section — add a print-specific class to body so
// print.css can hide everything else, then remove it after the dialog closes.
function previewPrint() {
  document.body.classList.add('print-invoice-only');
  window.print();
  // Remove the class after a brief delay (print dialog has closed or been dismissed)
  setTimeout(() => document.body.classList.remove('print-invoice-only'), 500);
}
